...
//            $apiTaskModel->siteid = $siteId;
                ...
                if(empty($maxStartTime))
                    $startTime = date('Y-m-d\TH:i:s.000\Z',strtotime(' -10 days') - 32400);
                else
                    $startTime = date('Y-m-d\TH:i:s.000\Z',strtotime($maxStartTime) - 28860);
                $nowTime = date('Y-m-d\TH:i:s.000\Z',time()-28800);
                $endTime = date('Y-m-d\TH:i:s.000\Z',strtotime($startTime)-7200);
                
                if(strcmp($endTime ,$nowTime) > 0)
                    $endTime = $nowTime;
                
//                if($account == 52){
//                    $startTime = '2018-09-17T23:29:14.329Z';
//                    $endTime = '2018-09-17T24:29:11.329Z';
//                }
                ...
                if(strcmp($endTime,$startTime) > 0)
                {
                    ...
                    //$getMyMessagesModel->MessageID = ['101411618809'];
//                    $getMyMessagesModel->siteID = $siteId;
            ...
            if (!empty($accountTask))
            {
                ...
                $accountTask->delete();
                VHelper::throwTheader('/services/ebay/getmymessages/mail', ['id'=> $accountId]);
            }
            exit('DONE');
        }
        else
        {
            ...
            if(!empty($accountList))
            {
                ...
                {
                    if(AccountTaskQueue::find()->where(['account_id'=>$account->id,'type'=>AccountTaskQueue::TASK_TYPE_MESSAGE,'platform_code'=>$account->platform_code])->exists())
                        continue;
            ...
            $taskList = AccountTaskQueue::getTaskList(['platform_code'=>Platform::PLATFORM_CODE_EB,'type'=>AccountTaskQueue::TASK_TYPE_MESSAGE]);
            if (!empty($taskList))
            {
