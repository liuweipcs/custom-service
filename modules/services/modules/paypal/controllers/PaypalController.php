        ...
        foreach ($refund_account_info as $key => $value) {
            ...
            if($thread)
            {
                if(time() - strtotime($thread->create_time) >= self::THREAD_TIME)
                {
                    ...
                    $thread->save();
                    VHelper::throwTheader('/services/paypal/paypal/index', ['account_id'=> $value['id']]);
                }
                else
                {
                    continue;
...
//        }while($result === false && $count<self::GET_DETAIL_COUNT );
        ...
        if ($result === false) {
            ...
            {
                $count++;
                VHelper::throwTheader('/services/paypal/paypal/index',['account_id'=>$account_id,'time_offset'=>$time_offset/2,'count'=>$count]);
            }
            exit($message);
        }
...
//                }
        ...
        $result_task_success = $this->saveTaskStatus($task_model, Task::EXECUTE_SUCCESS, 'excute success '.$paypal_count);
