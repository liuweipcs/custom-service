        ...
        <div class="col-lg-12">
            <div class="well">
                <input type="hidden" name="platform_code"
                       value="<?php echo \app\modules\accounts\models\Platform::PLATFORM_CODE_WALMART; ?>">
                            ...
                            <div class="form-group"><label class="control-label col-lg-5" for="">查询条件</label>
                                <div class="col-lg-7"><select class="form-control" name="condition_option"
                                        ...
                                        <option value="order_number"
                                                <?php if ($condition_option == 'order_number') { ?>selected<?php } ?>>
                                            order_number
                                        </option>
        ...
        <table class="table table-striped table-bordered">
            ...
            <?php if (!empty($orders)) { ?>
                <?php foreach ($orders as $item) { 
                    $account_info = \app\modules\accounts\models\Account::getHistoryAccountInfo($item['account_id'], 'WALMART');
                    ?>
                    <tr>
                        ...
                        <td><?php echo $item['buyer_id']; ?><br/>
                            <?php
                            if ($item['platform_code'] == "WALMART") {
                                $accountId = '';
                                $account = Account::findOne(['old_account_id' => $item['account_id'], 'platform_code' => Platform::PLATFORM_CODE_WALMART]);
                                if (!empty($account)) {
                                    $accountId = $account->id;
                                }
                                echo '<a href="' . Url::toRoute(['/mails/walmartreply/getsendemail', 'account_id' => $accountId, 'toemail' => $item['email'], 'platform_order_id' => $item['platform_order_id']]) . '" target="_blank"> 联系买家</a>';
                            }
                            ?>
                        </td>
                        <td>
                        ...
                        <td>
                            <div class="btn-group btn-list">
                                ...
                                <ul class="dropdown-menu" rol="menu">
                                    <?php if (in_array($item['complete_status'], [0, 1, 5, 10, 13, 15, 17, 25])) { ?>
