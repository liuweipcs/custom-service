                        ...
                        <div class="panel-body">
                            <label class="checkbox-inline">
                                <input name="after_sales_type[]" type="checkbox" id="refund-input"
                                       value="<?php echo AfterSalesOrder::ORDER_TYPE_REFUND ?>" <?php if (in_array($info['info']['order_type'], array(Order::ORDER_TYPE_MERGE_MAIN, Order::ORDER_TYPE_SPLIT_CHILD))) echo 'disabled="disabled"'; ?>>退款
                            </label>
                            <label class="checkbox-inline">
                                <input name="after_sales_type[]" type="checkbox" id="return-input"
                                       value="<?php echo AfterSalesOrder::ORDER_TYPE_RETURN ?>"<?php if ((in_array($info['info']['order_type'], array(Order::ORDER_TYPE_MERGE_RES, Order::ORDER_TYPE_SPLIT_MAIN))) || ($info['info']['complete_status'] == 40) || ($info['info']['complete_status'] < 19)) echo 'disabled="disabled"'; ?>>退货
                            </label>
                            <label class="checkbox-inline">
                                <input name="after_sales_type[]" type="checkbox" id="redirect-input"
                                       value="<?php echo AfterSalesOrder::ORDER_TYPE_REDIRECT ?>" <?php if ((in_array($info['info']['order_type'], array(Order::ORDER_TYPE_MERGE_RES, Order::ORDER_TYPE_SPLIT_MAIN))) || ($info['info']['complete_status'] == 40) || ($info['info']['complete_status'] < 19)) echo 'disabled="disabled"'; ?>>重寄
                            </label>
                            <label class="checkbox-inline">
                                <?php
                            ...
                            <label class="checkbox-inline" style="color: red;">
                                <?php
                                if (in_array($info['info']['order_type'], array(Order::ORDER_TYPE_MERGE_MAIN, Order::ORDER_TYPE_SPLIT_CHILD))) {
                                    echo '(合并后订单或者拆分后的子订单不能建立退款单)';
                                } elseif (in_array($info['info']['order_type'], array(Order::ORDER_TYPE_MERGE_RES, Order::ORDER_TYPE_SPLIT_MAIN))) {
                                    echo '(被合并的单或被拆分的单不能建立重寄单)';
                                }

                                if ($info['info']['complete_status'] == 40) {
                                    echo '(已作废订单不能创建重寄单)';
                                }

                                if ($info['info']['complete_status'] < 19) {
                                    echo '(未发货订单不能创建重寄单)';
                                }
                                ?>
                            </label>
                        ...
                        <div class="panel-body">
                            <?php if (!empty($reasonCodeList)) { ?>
                                <div class="row" style="display: <?php echo $platform == 'ALI' ? 'none' : 'black'; ?>">
                                    <div class="col-sm-10">
                                        <div class="form-group">
                                            <label for="ship_street1" class="col-sm-3 control-label">平台退款原因：<span
                                                    class="text-danger">*</span></label>
                                            <div class="col-sm-9">
                            ...
                            <div class="row">
                                <div class="col-sm-10">
                                    <div class="form-group">
                                        <label for="ship_name" class="col-sm-3 control-label required">退款金额：<span
                                                class="text-danger">*</span></label>
                                        <div class="col-sm-2">
                                            <!-- <input class="form-control" type="text" name="refund_amount" value="<?php //echo $orderAmount;  ?>" />-->
                                            <input class="form-control" type="text" name="refund_amount"
                                                   value="<?php echo $allow_refund_amount; ?>"/>
                                        </div>
                                        <div class="col-sm-2">
                                            <input class="form-control" type="text" disabled="disabled"
                                                   name="currency_code" value="<?php echo $currencyCode; ?>"/>
                                        </div>
                                        <div>
                                            <label class="control-label"
                                                   style="color:red">可退款金额：<span><?php echo $allow_refund_amount; ?></span></label>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    $('input#redirect-input').click(function () {
        var new_final_profit = "<?php echo $info['profit']['profit_new1']; ?>";

        if (this.checked) {
            if (new_final_profit < 0) {
                layer.confirm('此单负利润，确认是否要重寄！', {icon: 5, title: '警告', cancel: function (index, layero) {
                        layer.close(index);
                        $('input#redirect-input').removeAttr('checked');
                    ...
                    success: function (data) {
                        switch (data.status) {
                            ...
                            case 'success':
                                ...
                                $("input[name=return_rma]").attr('readonly', 'readonly');
                        }
                    }
                });
                layer.close(index);
    ...
    $("#search_payapl_record").on('click', function () {
        ...
        var account_id = $("#paypal_email").val();
