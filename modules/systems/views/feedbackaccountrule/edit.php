...
<div class="popup-wrapper">
    ...
    $form = ActiveForm::begin([
                'id' => 'updFeedbackAccountRuleForm',
                'action' => Url::toRoute(['/systems/feedbackaccountrule/edit']),
                'method' => 'post',
    ]);
    ?>
    <div class="popup-body">
        <table id="addFeedbackAccountRule">
            ...
            <tr>
                <td class="col1">账号：</td>
                <td>
                    <input type="radio" name="account_type" value="all" <?php
                    if ('all' == $info['account_type']) {
                        echo "checked";
                    }
                    ?>>所有账号
                    &nbsp;&nbsp;&nbsp;&nbsp;
                    <input type="radio" name="account_type" value="custom" <?php
                    if ('custom' == $info['account_type']) {
                        echo "checked";
                    }
                    ?>>指定账号
                    <div id="account_list">
                        <?php if (!empty($accountIdArr)) { ?>
                            ...
                            <?php foreach ($shortNameList as $item) { ?>
                                <div class="account_item"><input type="checkbox" name="account_ids[]" value="<?php echo $item['id']; ?>" <?php
                                    if (in_array($item['id'], $accountIdArr)) {
                                        echo "checked";
                                    }
                                    ?>><?php echo $item['account_name']; ?></div>
                                <?php } ?>
                            <?php } ?>
                    </div>
                </td>
            ...
            <tr>
                ...
                <td>
                    <input type="radio" name="msg_type" value="rand_centent" <?php if ($info['feedback_id']) { ?> checked <?php } ?>>随机发送留评模板
                    &nbsp;&nbsp;&nbsp;&nbsp;
                    <input type="radio" name="msg_type" value="centent" <?php if ($info['feedback_id']) { ?> checked <?php } ?>>指定内容  
                    <div class="centent" <?php if (empty($info['feedback_id'])) { ?> style="display: none;" <?php } ?>>
                        <select name="feedback_template_id" class="form-control">  
                            <option value="">选择回评模板</option>
                            <?php foreach($feedbacktemp as $v){ ?>
                            <option value="<?php echo $v->id;?>" <?php if($v->id==$info['feedback_id']){ $content=$v->template_content;  ?> selected <?php } ?> ><?php echo $v->template_content;?></option>
                             <?php } ?>
                        </select>
                        <textarea class="form-control" id="centent" style="height:119px;" name="centent"><?php echo $content; ?></textarea>
                    </div>
                </td>
            </tr>
            <tr>
