...
<div class="popup-wrapper">
    ...
    <div class="popup-body">
        ...
        <div class="row">
            <div class="col-sm-12">
                <?php echo $form->field($model, 'category_id')->dropdownList($categoryList, ['id' => 'category_id']); ?>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-sm-12" id="codeofplat">
                <?php echo $form->field($model, 'moban_replacement')->dropdownList($replaceList, ['onchange' => 'addbianliang(this)']); ?>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    function addbianliang(obj) {
        var valcode = $(obj).val();
        var valofcon = $('#mailtemplate-template_content').val();
        $.post("<?php echo Url::toRoute('/mails/msgcontent/getvar'); ?>", {
            "valcode": valcode
        }, function (data, status) {
            if (status) {
                addcon = valofcon + ' ' + data;
                $('#mailtemplate-template_content').val(addcon);
            }
        });
    }
</script>