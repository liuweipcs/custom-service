...
<script type="application/javascript">
    $(function(){
        ...
        function imageLarge(obj) {
            var obj = $(obj);
            var gridTbody = obj.clone();
            gridTbody[0].onmouseover = null;
            gridTbody[0].onclick = null;
            var objOffset = obj.offset();
            if(self != top)
            {
                $(window.frameElement).parents('.layui-layer-iframe').append(gridTbody);
                var iframeOffset = $(window.frameElement).offset();
                gridTbody.offset({'top':iframeOffset.top + (objOffset.top - document.body.scrollTop),'left':objOffset.left+iframeOffset.left});
            }
            else
            {
                $('body').append(gridTbody);
                gridTbody.offset(objOffset);
            }
            gridTbody.addClass('ebay_iamge_largen');
            gridTbody.css({'z-index': '9999','-webkit-transform': 'scale(5)','-moz-transform':'scale(5)','transform': 'scale(5)'});
            gridTbody.mouseout(function(){
                $(this).remove();
            });
        ...
        $('.ebay_return_image').mouseover(function(){
            imageLarge(this);
        });
        ...
        <div class="tab-content">
            <div id="home" class="tab-pane fade in active">
                <table class="table table-bordered">
                    ...
                    <tr>
                        <th>Return Id</th>
                        <th><?=$model->return_id?></th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr>
                        <td><?=$model->attributeLabels()['return_reason']?></td>
                        <td><?php echo $model->return_reason;?></td>
                    </tr>
                    <tr>
                        <td><?=$model->attributeLabels()['current_type']?></td>
                        <td><?php echo $model->current_type;?></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td><?=$model->attributeLabels()['return_creation_date']?></td>
                        <td><?php echo $model->return_creation_date;?></td>
                    </tr>
                    <tr>
                        <td><?=$model->attributeLabels()['return_comments']?></td>
                        <td><?php echo $model->return_comments;?></td>
                    </tr>
                    <tr>
                        <td>售后单号</td>
                        <td>
                ...
                <?php if(!empty($detailModel)):?>
                    处理过程
                    <?php if(empty($inbox_info))
                    {
                        echo '往来信息未更新！';
                    }
                    else if($inbox_info->inbox_subject_id == 0)
                    {
                        echo '<a target="_blank" href="/mails/ebayinbox/detail?id='.$inbox_info->id.'">查看邮件</a>';
                    }
                    else
                    {
                        echo '<a target="_blank" href="/mails/ebayinboxsubject/detail?id='.$inbox_info->inbox_subject_id.'">查看邮件</a>';
                    }
                    ?>
                <ul class="list-group">
                    <?php foreach($detailModel as $key => $detail):?>
                        <li class="list-group-item">
                            <?php echo isset($detail->creation_date_value) ? date('Y-m-d H:i:s',strtotime($detail->creation_date_value)+28800):'','&nbsp;&nbsp;&nbsp;&nbsp;<span style="color:#FF7F00">',$detail->author,'</span>&nbsp;&nbsp;&nbsp;&nbsp;',$detail->activity;?>
<!--                            <table class="table table-bordered">
                                <tbody>
                    ...
                    {
                        $returnImages = \app\modules\mails\models\EbayReturnImage::find()->select('submitter,GROUP_CONCAT(file_path SEPARATOR "{$}") file_paths')->where(['return_id'=>$model->return_id,'file_status'=>'PUBLISHED'])->groupBy('submitter')->asArray()->all();
                        foreach($returnImages as $returnImage)
                        {
                            echo '<li class="list-group-item"><div style="color:#C71585"><span>图片上传方：</span><span>'.$returnImage['submitter'].'</span></div><div>';
                            $imagesPath = explode('{$}',$returnImage['file_paths']);
                            foreach($imagesPath as $imagePath)
                            {
//                                echo '<img class="ebay_return_image" width="70px" height="70px" src="/'.$imagePath.'" alt="">';
                ...
                $buyer_id = $model->buyer_login_name;
                ...
                <?php if( $model->state != 'CLOSED' ):?>
                    <div class="popup-wrapper">
                        ...
                        <div class="popup-body">
                            <div class="row">
                                <input class="auto_refund_after_case_actual" type="hidden" name="EbayReturnsRequests[auto_refund]" value="<?=$model->auto_refund?>"/>
                                <input type="hidden" name="EbayReturnsRequestsResponse[ship_cost]" value="<?=$info['info']['ship_cost']?>"/>
                                <input type="hidden" name="EbayReturnsRequestsResponse[subtotal_price]" value="<?=$info['info']['subtotal_price']?>" />
                                <div>
                                    ...
                                    <div class="type_map_params">
                                        ...
                                        退款原因：
                                        <select name="EbayReturnsRequestsResponse[reason_code]">
                                            <?php foreach($reasonCode as $key =>$value){?>
                                                <option value="<?=$value->id?>"><?=$value->content?></option>
                                            <?php }?>
                                        </select><br/>
                                        退款金额：<input style="color:red;width: 40px;" readonly type="text" name="EbayReturnsRequestsResponse[currency]" value="<?=$model->currency?>"><input style="color:red" readonly type="text" name="EbayReturnsRequestsResponse[refund_amount][2]" value="<?=$model->buyer_estimated_refund_amount?>"/>
                                        <br/>
                                        <textarea name="EbayReturnsRequestsResponse[content][2]" rows="5" cols="50"></textarea>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="type_map_params">
                                        ...
                                        退款金额:<?=$model->currency?><input type="text" name="EbayReturnsRequestsResponse[refund_amount][3]" step="0.01"/>
                                        <br/>
                                        <textarea name="EbayReturnsRequestsResponse[content][3]" rows="5" cols="50"></textarea>
                                    </div>
                                </div>



                                <?php if($model->state == 'RETURN_REQUESTED' || $model->status == 'RETURN_REQUESTED'):?>
                                <div>
                                    <input type="radio" name="EbayReturnsRequestsResponse[type]" value="1" >发送留言
                                    <div class="type_map_params">
                        ...
                        <a class="btn btn-default mail_template_search_btn">搜索</a>
                                        ...
                                        <!--<textarea id="leave_message" name="EbayReturnsRequestsResponse[content][1]" rows="5" cols="50"></textarea>-->
                                        
                                        
                                        <?php echo Html::hiddenInput('sl_code',"",['id'=>'sl_code']);?>
                                        <?php echo Html::hiddenInput('tl_code',"",['id'=>'tl_code']);?>
                                        <div><textarea id='leave_message' name="EbayReturnsRequestsResponse[content][1]" rows="6" cols="180"></textarea></div>
                                        ...
                                        <div><textarea id='leave_message_en' name="EbayReturnsRequestsResponse[content][1_en]" rows="6" cols="180"></textarea></div>
