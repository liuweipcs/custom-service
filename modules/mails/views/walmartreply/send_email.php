...
<div id="page-wrapper">
    ...
    $form = ActiveForm::begin([
        'id' => 'sendEmailForm',
        'action' => Url::toRoute(['/mails/walmartreply/sendemail']),
        'method' => 'post',
    ...
    <table id="sendEmail">
        <tr>
            <td class="col1">SKU：</td>
            <td>
                <?php
                echo "<select name='sku' class='form-control'>";
                echo "<option value='all'>全部</option>";
                if (!empty($platformOrderId)) {
                    $order_info = OrderKefu::getOrderStack(Platform::PLATFORM_CODE_WALMART, $platformOrderId);
                    if (!empty($order_info)) {
                        $skus = OrderOtherDetail::find()
                            ->select('sku')
        ...
        <tr>
            ...
            <td>
                ...
                $tagList = Tag::find()
                    ...
                    ->andWhere(['platform_code' => Platform::PLATFORM_CODE_WALMART, 'status' => 1])
        ...
        <tr>
            ...
            <td>
                <div id="mailTemplateArea">
                    <?php
                    $templates = MailTemplate::getMyMailTemplate(Platform::PLATFORM_CODE_WALMART);
                    if (!empty($templates)) {
...
<script type="text/javascript">
    ...
    $('.mailTemplateSearch').click(function () {
        ...
        $.post('<?php echo Url::toRoute(['/mails/walmartreply/searchmailtemplate']); ?>', {
            "name": name
        }, function (data) {
            if (data["code"] == 1) {
                var data = data["data"];
